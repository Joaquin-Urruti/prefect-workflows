...
- [Features](#features)
- [Security Configuration for Production](#️-security-configuration-for-production)
...
- 🚀 **Production-Ready Setup**: Complete Docker Compose orchestration with health checks
- 💾 **Full Data Persistence**: PostgreSQL, Redis, logs, and outputs persisted on disk
- 🔄 **Automatic Scheduling**: Cron-based and interval-based workflow scheduling
- 📊 **Built-in Monitoring**: Web UI for real-time monitoring and debugging
- 🐳 **Containerized**: Isolated environment with all dependencies included
- 🔧 **Easy Configuration**: Environment variables and volume mappings
- 📝 **Comprehensive Logging**: Centralized logs with automatic rotation
- 🎯 **Process Worker**: Local worker pool for reliable task execution
...
This project uses environment variables for sensitive configuration. **Before deploying to production**, follow these steps:
...
The `.env` file is already listed in `.gitignore`. Verify it's not being tracked:
...
This means:
- ✅ **With `.env` file**: Uses your secure password from the file
- ⚠️ **Without `.env` file**: Falls back to the weak default `prefect`

### Security Best Practices

| Environment | Recommendation |
|-------------|----------------|
| **Development** | Default values are acceptable for local testing |
| **Production** | Always create a `.env` file with strong passwords |
| **CI/CD** | Use secrets management (GitHub Secrets, Vault, etc.) |
...
| `docker-compose.yml` | Service configuration with defaults | ✅ Yes |
...
│  └─────────────────────────────────────────────────────────┘ │
│                                                             │
│  Persistent Storage (Host Directories):                    │
│  ./data/postgres/  ./data/redis/  ./logs/  ./outputs/     │
└─────────────────────────────────────────────────────────────┘
```
...
|-----------|-------------|------|--------------|
| **PostgreSQL** | Stores Prefect metadata, flow runs, task states | 5432 | ✓ |
| **Redis** | Message broker for real-time communication | 6379 | ✓ |
| **Prefect Server** | REST API and web UI | 4200 | ✓ |
| **Prefect Services** | Background services (scheduler, etc.) | - | - |
| **Prefect Worker** | Executes workflows from work pool | - | ✓ |
...
├── .history/                   # Local history (gitignored)
...
│   ├── test.py                # Example deployment script
│   └── [your_workflows].py    # Your custom workflows
...
├── prefect-user-guide-en.md   # User guide (English)
├── prefect-user-guide-sp.md   # User guide (Spanish)
...
For production, make sure to change `PREFECT_POSTGRES_PASSWORD` to a strong, unique password.

> 💡 **Development**: You can skip this step for local testing (defaults will be used).
...
**That's it!** Your Prefect environment is ready. 🎉
...
2. **Configure environment variables**:
   ...
   Update `PREFECT_POSTGRES_PASSWORD` with a strong password:
   ```env
   PREFECT_POSTGRES_USER=prefect
   PREFECT_POSTGRES_PASSWORD=your_secure_random_password_here
   PREFECT_POSTGRES_DB=prefect
...
2. Change image version: `prefecthq/prefect:3-python3.13` → `prefecthq/prefect:X-python3.13`
